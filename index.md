...
Informações para a Saúde (RIPSA) no R. O pacote baixa os arquivos
publicados no Portal de Dados Abertos do SUS e retorna os resultados
como tibbles padronizadas, com colunas consistentes para código do
indicador, período, território, valor e categoria.
...
Os dados são baixados diretamente dos recursos CKAN do portal. O pacote
não depende de arquivos locais de indicadores nem de repositórios
externos como Zenodo.
...
geográfica, a cobertura temporal e as categorias disponíveis em cada
arquivo de origem.
...
Também é possível usar nomes legíveis de categoria quando eles aparecem
em
...
Para indicadores com categorias, o padrão `category = NULL` mantém todas
as categorias presentes no arquivo de origem. Em arquivos municipais
grandes, use `geo_code`, `category` ou `category_id` para reduzir o
volume baixado e tratado.
