...
Pacote R para recuperacao de indicadores da RIPSA como tibbles padronizadas.
...
`list_indicators()` inclui a cobertura disponível em cada arquivo de origem:
...
`get_indicator()` retorna uma tibble com colunas consistentes para qualquer
nivel geografico:
...
Use `list_categories()` para consultar categorias disponíveis e filtre com
`category_id`, `category_code` ou `category_name`:
